...
\]
where $\{\chi(h,.)\}$ are the characters (complex exponential homomorphisms) of $G$. When $G = \mathbb{Z}_N$ (the integers mod $N$), the QFT takes $|j\rangle$ to:
\[
\frac{1}{\sqrt{N}}\sum_{k=0}^{N-1} e^{2\pi i j k / N} |k\rangle
\] 

Let $G \cong \mathbb{Z}_{N_1} \oplus \cdots \oplus \mathbb{Z}_{N_k}$ be a decomposition of $G$. The character $\chi(\bm{a}, x)$ is then given by
\[
\chi(\bm{a}, \bm{x}) = \omega_{N_1}^{a_1x_1} \cdots \omega_{N_k}^{a_kx_k}, \quad \text{where } \omega_M = \exp(2\pi i/ M).
\]
...
\[
\qht_N: \ket{a} \mapsto \frac{1}{\sqrt{N}} \sum_{y = 0}^{N - 1} \cas\left(\frac{2\pi a y}{N}\right) \ket{y}, \label{eq:qht_def}
\]
...
\begin{equation}
    \qht(f)({\bm{a}}) = \frac{1}{\sqrt{|G|}} \sum_{{\bm{y}} \in G} \cas(2\pi \langle {\bm{a}}, \alpha({\bm{y}}) \rangle) f({\bm{y}}),
\end{equation}
where $\alpha({\bm{y}}) = y_1 / N_1 + \cdots + y_k / N_k$.

\paragraph{Unitary Equivalence.}
Using the identity
\[
\cas(2\pi \langle {\bm{a}}, \alpha({\bm{y}}) \rangle) = \frac{1 - i}{2}\chi({\bm{a}}, {\bm{y}}) + \frac{1 + i}{2}\chi(-{\bm{a}}, {\bm{y}}),
\]
we obtain:
\begin{equation}
\qht_G = \frac{1 - i}{2} \qft_G + \frac{1 + i}{2} \qft_G^*, \label{eq:QHT_QFT}
\end{equation}
showing that $\qht_G^2 = {1}$ and hence the $\qht$ is unitary.

\paragraph{Quantum Formulation.}
The quantum Hartley transform over $G$ maps:
\[
\sum_{{x} \in G} f({x}) \ket{{x}} \mapsto \sum_{{\bm{a}} \in G} \qht_G(f)({\bm{a}}) \ket{{\bm{a}}}.
\]
